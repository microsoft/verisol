...
val size_union2: #a:eqtype -> #f:cmp a -> s:ordset a f -> k:a 
                -> Lemma (requires True)
                         (ensures not (mem k s) ==> size (union s (singleton k)) == size s + 1)
                         [SMTPat (size #a #f (union #a #f s k))]
                      
val size_union3: #a:eqtype -> #f:cmp a -> s:ordset a f -> k:a 
                -> Lemma (requires True)
                         (ensures (mem k s) ==> size (union s (singleton k)) == size s
                                                /\ equal s (union s (singleton k))
                         )
                         [SMTPat (size #a #f (union #a #f s k))]
...
val non_zero_size_choose: #a:eqtype -> #f:cmp a -> s:ordset a f ->
                          Lemma
                          (requires size s > 0)
                          (ensures Some? (choose s))

val choose_upd_det: #a:eqtype -> #f:cmp a -> s:ordset a f -> x:a 
              -> Lemma (requires (not (s = (empty #a #f))))
                       (ensures (     Some? (choose #a #f s) /\ 
                                ( size_union s (singleton x); non_zero_size_choose s;
                                  (x <> (Some?.v (choose #a #f (union #a #f s (singleton x)))) ) ==> (
                                  //(not (f x (Some?.v (choose #a #f (union #a #f s (singleton x)))))) /\ 
                                ( (Some?.v (choose #a #f (union #a #f s (singleton x)))) = (Some?.v (choose #a #f s)) )                       
                        )
                        )))
                //[SMTPat (choose #a #f s); SMTPat (choose #a #f (union #a #f s (singleton x)))]

val choose_upd_det2: #a:eqtype -> #f:cmp a -> s:ordset a f -> x:a 
              -> Lemma (requires (not (s = (empty #a #f))))
                       (ensures (     Some? (choose #a #f s) /\ 
                                ( size_union s (singleton x); non_zero_size_choose s;
                                  ( (mem x s) /\  x <> (Some?.v (choose #a #f s)) ) ==> (
                                  //(not (f x (Some?.v (choose #a #f (union #a #f s (singleton x)))))) /\ 
                                ( (Some?.v (choose #a #f (union #a #f s (singleton x)))) = (Some?.v (choose #a #f s)) )                       
                        )
                        )))
...
val del_union_choose: #a:eqtype -> #f:cmp a -> s:ordset a f -> x:a 
              -> Lemma (requires (not (s = (empty #a #f))))
                       (ensures ( Some? (choose #a #f s) /\ 
                                ( size_union s (singleton x); non_zero_size_choose s; choose_upd_det s x; 
                                  let k'' = (Some?.v (choose #a #f s)) in let k' = (Some?.v (choose #a #f (union #a #f s (singleton x)))) in 
                                  (x <> k' /\ k' = k'') ==> (
                                    equal (remove k'' s) (remove k' (union #a #f s (singleton x)) )
                                  )
                                )
                       )) //It might go thru if you write expressions completely instead of letting them.
              [SMTPat (choose #a #f s); SMTPat (choose #a #f (union #a #f s (singleton x)))] //FIXME: Pattern may be insufficient.
